...
export default {
	...
	methods: {
		...
		newOperatingRegion: async function(participantId) {
			...
			const countryId = this.returnDetails(this.operatingRegion.country.listMapDetails, "name", this.operatingRegion.country.selected, "id");
			const cityId = this.returnDetails(this.operatingRegion.country.city.listMapDetails, "name", this.operatingRegion.country.city.selected, "id");
			...
			axios
					.post(api_map.singleNetworkParticipant + participantId + api_map.getNetworkRoleDetails + this.networkInfo.id + api_map.newOperatingRegion,
							{
